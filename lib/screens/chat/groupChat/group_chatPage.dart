...
class _GroupChatPageState extends State<GroupChatPage> {
            ...
            onTap: () async {
              if (chatController.typeMessageCon.value.text.isNotEmpty) {
                chatController.sendMessageForGroupChat(
                    ...
                    groupMembers: chatController.groupInfo.value.membersId);
          ...
          child: PopupMenuButton(
              ...
              itemBuilder: (context) {
                return [
                  ...
                  PopupMenuItem<int>(
                    value: 2,
                    child: Text(chatController.groupInfo.value.isArchive
                            .toString()
                            .contains(authController.userModel.value.user?.id
                                    .toString() ??
                                '')
                        ? 'Remove from Archive'
                        : 'Add to Archive'),
              ...
              onSelected: (value) async {
                ...
                if (value == 0) {
                  ...
                  return showDialog(
                    ...
                    builder: (context) => AlertDialog(
                      title: Text(
                          chatController.groupInfo.value.isArchive
                                  .toString()
                      ...
                      actions: [
                        AppWidget.elevatedButton(
                            text: 'Yes',
                            onTap: () {
                              if (chatController.groupInfo.value.isArchive
                                  .toString()
                                  .contains(authController
                                          .userModel.value.user?.id
                                          .toString() ??
                                ...
                                chatController.groupChatRoomCollection
                                    .doc(chatController.groupInfo.value.groupId
                                        .toString())
                                    .update({
                                  ApiConstant.isArchive: FieldValue.arrayUnion([
                                    authController.userModel.value.user?.id
                                        .toString()
                                  ]),
                                }).then((value) {
                                  Get.back();
                                  chatController.groupInfo.value.isArchive?.add(
                                      authController.userModel.value.user?.id
                                          .toString());
                                });
                              }
  ...
  Widget buildItem(int index, DocumentSnapshot? document) {
    if (document != null) {
      ...
      if (messageChat.idFrom.toString() ==
          authController.userModel.value.user?.id.toString()) {
        // Right (my message)
        ...
        if (messageChat.isReadForGroup?.isEmpty == true &&
            messageChat.isRead == false) {
          chatController.updateMessageReadStatusForGroup(messages: messageChat);
        } else {
          chatController.removeIntoIsReadForGroup(messages: messageChat);
        }
